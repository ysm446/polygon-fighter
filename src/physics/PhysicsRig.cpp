...
JPH::RVec3 PhysicsRig::SpawnPosition(const std::array<float, 3>& position) const {
    ...
    return root + offset_ + rotation_ * (Position(position) - root);
}
PhysicsRig::PhysicsRig(JPH::PhysicsSystem& system, JPH::RVec3 offset, float yaw, JPH::uint group)
    : system_(system), offset_(offset), rotation_(JPH::Quat::sRotation(JPH::Vec3::sAxisY(), yaw)) {
...
PhysicsRig::~PhysicsRig() {
    ...
    system_.RemoveConstraint(anchor_);
...
bool PhysicsRig::RequestPunch() { return animation_.RequestPunch(); }
